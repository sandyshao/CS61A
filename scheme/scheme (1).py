...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        value = scheme_eval(procedure.body, new_frame)
        return value
...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        for i in range(len(formals)):
            frame.define(list(formals)[i], list(vals)[i])
        return frame
...
def do_lambda_form(vals, env):
    ...
    if len(vals)==2:
        return LambdaProcedure(vals[0], vals[1], env)
    else:
        return LambdaProcedure(vals[0], Pair('begin',vals.second), env)
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        function_name = vals.first.first
        if not scheme_symbolp(function_name):
            raise SchemeError
        lambda_expression = Pair("lambda",(Pair(vals.first.second, vals.second)))
        env.define(function_name, scheme_eval(lambda_expression, env))
...
def do_if_form(vals, env):
    ...
    check_form(vals, 2, 3)
    procedure = vals.first
    true_eval = vals.second.first
    if vals.second.second != nil:
        false_eval = vals.second.second.first
    if scheme_true(scheme_eval(procedure, env)):
        return true_eval
    elif scheme_false(scheme_eval(procedure, env)):
        if vals.second.second == nil:
            return okay
        return false_eval
...
def do_and_form(vals, env):
    ...
    if vals != nil:
        ...
        while index < len(vals)-1:
            if scheme_false(scheme_eval(vals[index], env)):
                return False
            index+=1
...
def do_or_form(vals, env):
    ...
    if vals != nil:
        ...
        while index < len(vals):
            if scheme_true(scheme_eval(vals[index], env)):
                return quote(vals[index])
            if scheme_true(scheme_eval(vals[index], env)) and index==len(vals-1):
                return vals[index]
            index+=1
        return False
...
def do_cond_form(vals, env):
    ...
    for i, clause in enumerate(vals):
        ...
        if scheme_true(test):
            ...
            expression = Pair('begin',clause.second)
...
def do_begin_form(vals, env):
    ...
    print(vals)
    check_form(vals, 1)
    [scheme_eval(vals[index],env) for index in range(len(vals)-1)]
    return vals[len(vals)-1]
...
def check_formals(formals):
    ...
    for i in range(len(formals)):
        if not scheme_symbolp(formals[i]) or formals[i] in lst:
            raise SchemeError
        lst.append(formals[i])
