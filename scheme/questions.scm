...
(define (zip pairs)
  (cond 
    ((equal? pairs nil) (list (append (list)) (append ( list ))))
    (else (list (append (append (list(caar pairs)) (car(zip (cdr pairs))))) (append (list (cadar pairs)) (cadr(zip (cdr pairs))))))
          ))
...
(define (list-partitions total max-pieces max-value)
  ; *** YOUR CODE HERE ***
  (define (partitions accm total max-pieces max-value)
    (cond ((= total 0) (list accm))
      ((or (< total 0) (= max-pieces 0) (<= max-value 0)) '())
      (else (append (partitions
              (append accm (list max-value))
              (- total max-value)
              (- max-pieces 1)
              max-value)
              (partitions accm total max-pieces (- max-value 1))))))
  ...
  (cond ((atom? expr)
        ...
        ((or (lambda? expr)
         ...
         (let ((form   (car expr))
               (params (cadr expr))
               (body   (cddr expr)))
                (append(append (list 'lambda params) (analyze body)))
           ))
        ((let? expr)
         (let ((values (cadr expr))
               (body   (cddr expr)))
           (append (list(append (list 'lambda (car(zip values)) (analyze (car body))))) (analyze (cadr(zip values))))
           ))
        (else
